...
function* testmanan1_get_Pet_id_readWorker(action) {
  try {
    const result = yield call(apiService.testmanan1_get_Pet_id_read, action)
    yield put(actions.testmanan1_get_Pet_id_readSucceeded(result))
  } catch (err) {
    yield put(actions.testmanan1_get_Pet_id_readFailed(err))
  }
}
function* testmanan1_get_Pet_id_readWatcher() {
  yield takeEvery(
    types.TESTMANAN1_GET__PET_ID_READ,
    testmanan1_get_Pet_id_readWorker
  )
}
...
export default function* rootSaga() {
  const sagas = [
    testmanan1_get_Pet_id_readWatcher,
